...
public abstract class SharedXenoPunchSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        // SubscribeLocalEvent<XenoPunchComponent, ThrowDoHitEvent>(OnXenoPunchHit);
    }
    ...
    private void OnXenoPunchAction(Entity<XenoPunchComponent> xeno, ref XenoPunchActionEvent args)
    {
        ...
        if (xeno.Comp.ThrowStrength != 0)
        {
            _throwing.TryThrow(marineId, diff, xeno.Comp.ThrowStrength);
        }

        if (xeno.Comp.StunTime != TimeSpan.FromSeconds(0))
        {
            _stun.TryParalyze(marineId, xeno.Comp.StunTime, true);
        }
